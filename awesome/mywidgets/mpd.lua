...
vicious.register(mpdwidget, function()
		...
		if l == nul or l:find("MPD_HOST") or l:find("error: Connection refused") then
			...
			if not l:find("volume:") then
				...
				l = f:read()
				if l ~= null then
					str = str .. ' ' .. separator_text .. ' ' .. l:gsub('[^a-zA-Z]([a-zA-Z]*)[^a-zA-Z][ ]*#([0-9]*)[^0-9]*([0-9]*)[ ]*([0-9\\:]*)[^0-9\\:]*([0-9\\:]*).*', '%4 / %5' .. separator_text .. ' %1 ' .. separator_text .. ' #%2 / %3')
					l = f:read()
					if l ~= null then
						str = str .. ' ' .. separator_text .. ' ' .. l:gsub('[^r]*([^ ]* [^ ]*)[ ]*([^ ]* [^ ]*).*', '%1 ' .. separator_text .. ' %2')
					end
				end
			else
